...
// MARK: - Precomputed particle parameters rendered as functions of elapsed time
...
struct Burst {
  var at: Double  // Seconds since the celebration started.
  var fx: Double  // Emission x position as a fraction of the width.
  var fy: Double  // Emission y position as a fraction of the height.
  var color: Int
  var confetti: [ConfettiParams]
  var sparks: [SparkParams]
}

// MARK: - Explosion centered on the triggered cell: sparks, shockwave, and flash
...
// MARK: - Celebration with roughly two seconds of confetti and firework bursts
...
// MARK: - Effects overlay (Canvas and TimelineView run only while an effect is active)
...
struct EffectsOverlay: View {
  ...
  // Closed-form damped motion: v(t) = v0 * drag^(60t), displacement = v0 * (e^(kt) - 1) / k, k = 60 * ln(drag).
  ...
  private func drawExplosion(ctx: inout GraphicsContext, size: CGSize, fx: ExplosionFX, now: Date) {
    ...
    // White flash that expands and fades over 0.18 seconds.
    if t < 0.18 {
      let p = t / 0.18
    ...
    // Shockwave ring lasting 0.45 seconds.
    if t < 0.45 {
      let p = t / 0.45
    ...
    // Orange-red sparks spreading radially under gravity and drag.
  ...
  private func drawCelebration(
    ctx: inout GraphicsContext, size: CGSize, fx: CelebrationFX, now: Date
  ) {
    let t = now.timeIntervalSince(fx.start)
    guard t >= 0 else { return }
    for burst in fx.bursts {
      ...
      // Single-color firework sparks spreading radially.
      ...
      // Colorful confetti rising, drifting, and tumbling.
      for p in burst.confetti where bt < p.life {
        ...
        c.fill(
          Path(CGRect(x: -CGFloat(p.size), y: -h / 2, width: CGFloat(p.size) * 2, height: h)),
