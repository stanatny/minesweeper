...
// MARK: - Board, effects overlay, mouse bridge, and shake animation
      ...
      ForEach(0..<model.config.height, id: \.self) { y in
        HStack(spacing: 0) {
          ForEach(0..<model.config.width, id: \.self) { x in
            ...
            CellView(
              cell: model.cells[i], theme: theme,
...
struct CellView: View {
  let cell: CellState
  let theme: Theme
  var pressed: Bool = false  // Temporarily recess covered cells for chord previews and flashes.
  ...
  var body: some View {
    ZStack {
      if cell.revealed {
        ...
        // Hide the raised bevel during previews so the cell looks recessed.
...
// MARK: - Shake animation (GeometryEffect)
...
// MARK: - AppKit bridge for mouse events
// SwiftUI leaves right-click events unhandled; this container forwards them through callbacks.
...
final class RightClickHostView<Content: View>: NSView {
  ...
  override var isFlipped: Bool { true }  // Match SwiftUI coordinates with a top-left origin.
